...
export default async function PlayerProfilePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="space-y-7">
      ...
      {recentMatches.length > 0 && <section className="surface p-6"><div><p className="data-label">Последние катки</p><h2 className="mt-1 text-lg font-semibold tracking-[-0.04em] text-graphite">История матчей</h2></div><ul className="mt-5 space-y-3">{recentMatches.map((match) => <li key={match.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-hairline py-2.5 text-xs first:pt-0 last:border-none last:pb-0"><span className="font-semibold text-graphite">{match.heroName}</span><span className="text-graphite-muted">{match.kills}/{match.deaths}/{match.assists}</span><span className={match.win ? "text-accent-success" : "text-accent-danger"}>{match.win ? "Победа" : "Поражение"} · {formatDuration(match.match.duration)} · {formatDate(match.match.startTime)}</span></li>)}</ul></section>}
    </div>
  );
}
